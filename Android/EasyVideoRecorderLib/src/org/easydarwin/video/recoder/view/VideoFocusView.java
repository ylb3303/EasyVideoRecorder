...
public class VideoFocusView extends RelativeLayout {
	...
	public static final int FOCUS_IMG_WH = 160;
...
//	private DisplayMetrics displayMetrics;
	...
	public VideoFocusView(Context context, AttributeSet attrs) {
		...
		mFocusImg.setBackgroundResource(R.drawable.easy_video_recorder_image_focus);
		addView(mFocusImg);

		if (isShowFilpCamrea()) {
			mChangeCamera = new ImageView(context);
			mChangeCamera.setBackgroundResource(R.drawable.easy_video_recorder_chage_camera_arrow);
		...
		if (isShowGuide()) {
			...
			LayoutParams noticeParams = new LayoutParams(android.view.ViewGroup.LayoutParams.MATCH_PARENT, android.view.ViewGroup.LayoutParams.WRAP_CONTENT);
	...
	public boolean isShowFilpCamrea() {
		return mShowGrid && context.getSharedPreferences(VIDEO_SETTING_NAME, Context.MODE_PRIVATE).getInt(GUIDE_SHOWFILPCAMREA_KEY, 0) < 1;
	}
