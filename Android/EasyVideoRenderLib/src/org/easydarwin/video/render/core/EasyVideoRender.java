...
import org.easydarwin.video.render.render.ParamKeeper;
import org.easydarwin.video.render.render.RenderResHelper;
...
public class EasyVideoRender {
	...
	private RenderConfig renderConfig = RenderConfig.create();
...
//		LoggerOptions options = LoggerOptions.createSimple(this);
	...
	public void start() {
		if (prepare()) {
			...
			bundle.putSerializable("config", renderConfig);
	...
	private boolean prepare() {
		...
		File tmpDir = new File(renderConfig.getVideoOutputDir());
		if (!tmpDir.exists()) {
			if (!tmpDir.mkdirs()) {
	...
	public EasyVideoRender setRenderConfig(RenderConfig renderConfig) {
